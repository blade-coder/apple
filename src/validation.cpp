...
#if defined(NDEBUG)
# error "AppleCoin Core cannot be compiled without assertions."
#endif
...
bool CheckFoundersInputs(const CTransaction &tx, CValidationState &state, int nHeight){
    ...
    if(nHeight > (1000000-20) && nHeight < (1000000+20)){
        return true;
    }
    if(nHeight > (2000000-20) && nHeight < (2000000+20)){
        return true;
    }
    bool found_1 = false;

    CScript FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress("AXtn8yzAzR7yAchfzWQJrADARktc4e2ywv").Get());

    CAmount amount = GetBlockSubsidy(0,nHeight,Params().GetConsensus(),false);

    BOOST_FOREACH(const CTxOut &output, tx.vout)
    {
        if (output.scriptPubKey == FOUNDER_1_SCRIPT && output.nValue == (int64_t)(amount * 0.05))
        {
            found_1 = true;
            continue;
        }
...
{
    CAmount nSubsidy = blockValue * 0.3;
...
void ThreadScriptCheck() {
    RenameThread("AppleCoin-scriptch");
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // APPLE : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(APPLE): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(APPLE): couldn't find masternode or superblock payments"),
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // APPLE : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        BOOST_FOREACH(const CTransaction& tx, block.vtx) {
            ...
            BOOST_FOREACH(const CTxIn& txin, tx.vin) {
                ...
                if(instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx.GetHash()) {
                    ...
                    LOCK(cs_main);
                    mapRejectedBlocks.insert(make_pair(block.GetHash(), GetTime()));
                    return state.DoS(0, error("CheckBlock(APPLE): transaction %s conflicts with transaction lock %s",
        ...
        LogPrintf("CheckBlock(APPLE): spork is off, skipping transaction locking checks\n");
    }
