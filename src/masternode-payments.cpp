...
*
*   Why is this needed?
*   - In AppleCoin some blocks are superblocks, which output much higher amounts of coins
...
void FillBlockPayments(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet, std::vector<CTxOut>& voutSuperblockRet)
{
    ...
    mnpayments.FillBloc(txNew,nBlockHeight-1,blockReward);
    if(sporkManager.IsSporkActive(SPORK_9_SUPERBLOCKS_ENABLED) &&
...
void CMasternodePayments::FillBloc(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward){
    ...
    CAmount amout = GetBlockSubsidy(0,nBlockHeight,Params().GetConsensus(),false) * 0.05;
    txNew.vout[0].nValue = txNew.vout[0].nValue - amout;

    CScript FOUNDER_1_SCRIPT = GetScriptForDestination(CBitcoinAddress(FUNDER_ADDRESS).Get());
    txNew.vout.push_back(CTxOut(amout, CScript(FOUNDER_1_SCRIPT.begin(), FOUNDER_1_SCRIPT.end())));
...
{
    if(fLiteMode) return; // disable all AppleCoin specific functionality
...
bool CMasternodeBlockPayees::IsTransactionValid(const CTransaction& txNew)
{
    ...
    LogPrintf("CMasternodeBlockPayees::IsTransactionValid -- ERROR: Missing required payment, possible payees: '%s', amount: %f APPLE\n", strPayeesPossible, (float)nMasternodePayment/COIN);
    return false;
}
